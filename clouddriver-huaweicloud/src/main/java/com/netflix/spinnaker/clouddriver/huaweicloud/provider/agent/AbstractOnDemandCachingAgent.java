...
public abstract class AbstractOnDemandCachingAgent extends AbstractHuaweiCloudCachingAgent
  ...
  private static final Logger log = LoggerFactory.getLogger(AbstractOnDemandCachingAgent.class);
  ...
  public Collection<Map> pendingOnDemandRequests(ProviderCache providerCache) {
    ...
    Collection<String> keys =
        providerCache.getIdentifiers(ON_DEMAND.ns).stream()
            .filter(
                key -> {
                  Map<String, String> parsedKey = Keys.parse(key);
                  return parsedKey != null
                      && parsedKey.get("account") == getAccountName()
                      && parsedKey.get("region") == region;
                })
            .collect(Collectors.toList());

    return providerCache.getAll(ON_DEMAND.ns, keys).stream()
        .map(
            cacheData -> {
